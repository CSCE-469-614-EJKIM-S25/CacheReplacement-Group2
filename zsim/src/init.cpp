...
BaseCache *BuildCacheBank(Config &config, const string &prefix, g_string &name, uint32_t bankSize, bool isTerminal, uint32_t domain)
{
    ...
    else if (replType == "SHiP")
    {
        ...
        rp = new SHIPReplPolicy(rpvMax, numLines, SHCT_size);
    }
...
static void InitSystem(Config &config)
{
    ...
    // Check single LLC
    if (cMap[llc]->size() != 1)
        panic("Last-level cache %s must have caches = 1, but %ld were specified", llc.c_str(), cMap[llc]->size());
    ...
    { // trace-driven: create trace driver and proxy caches
...
static void PostInitStats(bool perProcessDir, Config &config)
{
    ...
    if (zinfo->statsPhaseInterval)
    {
        ...
        if (!prStat)
            panic("No stats match sim.periodicStatsFilter regex (%s)! Set interval to 0 to avoid periodic stats", periodicStatsFilter);
    ...
    if (zinfo->maxMinInstrs)
    {
        ...
        {
            auto getInstrs = [i]()
            { return zinfo->cores[i]->getInstrs(); };
            auto dumpStats = [i]()
            {
